...
In the poverty status dataset derived from the American Community Survey (ACS) for the years 2015 - 2020, several key variables are utilized to explore the socioeconomic landscape across different demographics. The dependent variable in this analysis is the poverty status, which is defined as the proportion of individuals below the poverty line in each surveyed group. The independent variables include the year of the survey, which ranges from 2015 to 2020, and demographic identifiers such as race and state.
