...
```{r final ACS Data}
desired_vars = c('B17001A_032','B17001A_001', 'B17001A_046','B17001B_001','B17001B_032','B17001B_046','B17001C_001','B17001C_032','B17001C_046','B17001D_001','B17001D_032','B17001D_046','B17001E_001','B17001E_032','B17001E_046','B17001F_001','B17001F_032','B17001F_046','B17001G_001','B17001G_032','B17001G_046','B17001H_001','B17001H_032','B17001H_046','B17001I_001','B17001I_032','B17001I_046')
...
poverty_totals <- poverty_final %>%
  group_by(NAME, race) %>%
  mutate(total = estimate[gender == "Total"]) |>
  mutate(proportion = estimate/total) |>
  filter(proportion != 1) |>
  select(-estimate, -NAME, -total) |>
  ungroup() |>
  pivot_wider(names_from = c(gender,race), values_from = proportion)



#### This data refers to the raw number of people in each state who live above the poverty line, fauceted by race and gender.
```



```{r map idea}
column_names <- names(poverty_totals)[-which(names(poverty_totals) == "geometry")]

for (x in column_names) {
  print(
    ggplot(data = poverty_totals, aes(fill = .data[[x]])) +
      geom_sf() +
      coord_sf(crs = 5070) +
      scale_fill_viridis_c(option = 'plasma', limits = c(.15,.62)) +
      theme_minimal() +
