...
```{r final ACS Data}
desired_vars = c('B17001A_032', 'B17001A_046','B17001B_032','B17001B_046','B17001C_032','B17001C_046','B17001D_032','B17001D_046','B17001E_032','B17001E_046','B17001F_032','B17001F_046','B17001G_032','B17001G_046','B17001H_032','B17001H_046','B17001I_032','B17001I_046')
...
total <- get_acs(
  ...
  select(-moe, -variable, -GEOID, -NAME, -geometry) |>
...
poverty_final <- get_acs(
  ...
  select(-moe, -variable, -GEOID) |>
  pivot_wider(names_from = c(gender, race), values_from = estimate) |>
  st_join(total, join = st_equals)
...
```
```{r map idea}
column_names <- poverty_final %>%
  select(-NAME, -geometry, -TOTAL) %>%
  names()

for (x in column_names) {
  print(
    ggplot(data = poverty_final, aes(fill = .data[[x]]/`TOTAL`)) +
      geom_sf() +
      coord_sf(crs = 5070) +
      scale_fill_viridis_c(option = 'plasma') +
      theme_minimal() +
